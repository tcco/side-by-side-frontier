Below is a C++11 SPSC ring buffer implementation. It is:

- Single-producer / single-consumer only.
- Non-blocking: `push`, `emplace`, and `pop` return `false` instead of waiting.
- Mutex-free / condition-variable-free.
- Uses explicit `memory_order_acquire` / `memory_order_release`.
- Uses cache-line padding/alignment so the read and write indices do not share a cache line.
- Allocates a fixed-capacity buffer once at construction time.
...
int main() {
    SpscRingBuffer<int> queue(1024);

    std::thread producer([&] {
        for (int i = 0; i < 100000; ++i) {
            while (!queue.push(i)) {
                // Non-blocking retry/spin.
            }
        }
    });
...
- Exactly one thread may call `push` / `emplace`.
- Exactly one thread may call `pop`.
- The destructor must not run concurrently with producer or consumer threads.
- `write_index_` is release-stored by the producer and acquire-loaded by the consumer.
- `read_index_` is release-stored by the consumer and acquire-loaded by the producer.
- This implementation uses monotonic sequence counters, so the usable capacity is exactly the requested capacity.