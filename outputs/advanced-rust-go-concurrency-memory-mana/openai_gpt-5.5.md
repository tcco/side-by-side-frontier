Below is a complete Go implementation of a thread-safe, memory-bounded asynchronous batch processor.

It provides:

- Multiple concurrent producers via `Submit` / `SubmitContext`
- Batching by size `N`
- Automatic flushing after timeout `T`
- Backpressure via a bounded internal channel of capacity `C`
- Safe shutdown that flushes all accepted items before terminating
- No channel-close send panics during concurrent shutdown/submission
...
- Producers call `Submit`.
- Internally, items go into a bounded channel with capacity `C`.
- If the channel is full, producers block, creating natural backpressure.
- A background worker goroutine consumes items and builds batches.
- A batch is flushed when:
  - It reaches size `N`, or
  - The oldest item in the current batch has waited for `T`.
- `Shutdown` safely closes the processor to new input.
- The worker drains everything already accepted, flushes the final partial batch, and then exits.
- `Shutdown` waits for that worker to finish, ensuring no accepted items are lost.