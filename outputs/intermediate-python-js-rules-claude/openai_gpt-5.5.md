Below is a complete functional TypeScript refactor. It avoids mutation, avoids classes, uses a State Monad for the billing pipeline, represents money internally with exact rational cents, and includes boundary-test logic for floating-point-to-currency conversion.
...
- Replaced mutation with immutable state transitions.
- Replaced ad hoc floating-point money math with exact rational cent arithmetic.
- Implemented the billing pipeline with a State Monad.
- Preserved grandfathered professional pricing.
- Preserved upgrade proration, but removed the hardcoded `30` days by requiring `daysInPeriod`.
- Preserved promo behavior:
  - `WINTER24`: 15% off.
  - `STARTUP_50`: `$50.00` off non-starter plans.
- Preserved region tax behavior.
- Preserved the original final rounding behavior by using ceiling-to-cent.
- Added boundary-safe floating-point currency conversion and test reporting.
- Returned an updated user object instead of mutating the input user.