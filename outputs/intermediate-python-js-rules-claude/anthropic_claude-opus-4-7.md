# Refactored SaaS Billing System (TypeScript)

Below is the complete, fully refactored TypeScript implementation following all your structural rules: purely functional, State Monad–driven, exhaustively typed, zero dependencies, and mathematically rigorous in its currency handling.
...
### 1. **Eliminated Mutation**
The original mutated `usr_obj['last_billed_amt']`. The refactor returns a new `IUser` via spread, leaving the caller's object frozen.

### 2. **State Monad Pipeline**
The pricing flow (`base → prorate → promo → clamp → tax`) is composed via `Sequence` over independent `IStateMonad<IBillingState, null>` steps. Each step is a pure function `S → {value, state}`.

### 3. **Integer-Cent Currency Math**
The Python version used `math.ceil(total * 100) / 100` as a "hack." The refactor stores all money in **integer cents** throughout, only converting to dollars at the boundary. This avoids classic traps:
- `79.99 * 100 === 7998.999999999999` → solved with epsilon-aware `ToCents`.
- `0.1 + 0.2 !== 0.3` → tested explicitly.

### 4. **Exhaustive Interfaces**
Every input/output (`IBillingRequest`, `IBillingState`, `IBillingResult`, `IBillingError`, `BillingOutcome`, `IStateResult`, `IStateMonad`, `ITestResult`) is declared with `readonly` fields.

### 5. **Discriminated Error Union**
Replaced the Python `"ERROR: invalid plan"` string return with `BillingOutcome = {ok:true,...} | {ok:false,...}` — type-safe and unambiguous.

### 6. **Naming Conventions**
- All exported functions: **PascalCase** (`CalculateBilling`, `ToCents`, `ApplyPromo`, `Bind`, `Sequence`).
- All variables/parameters: **camelCase** (`baseCents`, `targetPlan`, `isMidCycleUpgrade`).

### 7. **Boundary Tests Included**
`RunBoundaryTests()` exercises: round-tripping, floating-point sums, negative clamping, grandfathered logic, proration at both `0` and `cycleLength` boundaries, immutability, and validation errors.

### 8. **Zero Dependencies**
Uses only the standard library — no `fp-ts`, no `decimal.js`, no testing framework. The monad and tests are hand-rolled.

### Usage Example

```typescript
const outcome = CalculateBilling({
  user: { userId: "alice", grandfatheredYear: 2021 },
  targetPlan: "professional",
  region: "EU",
  isMidCycleUpgrade: true,
...
if (outcome.ok) {
  console.log(outcome.value.total, outcome.value.auditTrail);
}

console.table(RunBoundaryTests());
```