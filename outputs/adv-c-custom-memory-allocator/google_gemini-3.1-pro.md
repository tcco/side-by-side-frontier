Here is a complete, deterministic, pool-based memory allocator written in raw C. 

### Design Principles
1. **Memory Pool:** A statically allocated `uint64_t` array guarantees that the base memory pool is strictly 8-byte aligned.
2. **Block Structure:** Each block consists of an 8-byte **Header**, a **Payload** (multiple of 8 bytes), and an 8-byte **Footer**. 
3. **Boundary Tags:** The header and footer both store the block's size and its allocation status (using the lowest bit). Footers allow $O(1)$ backward traversal, which is required for immediate coalescing.
4. **Prologue & Epilogue:** To avoid edge-case bounds checking during coalescing, we place a dummy "Prologue" block at the start and an "Epilogue" block at the end. Both are marked as allocated so the coalescing logic naturally stops at the pool boundaries.
5. **First-Fit:** The allocator linearly scans from the beginning of the heap and selects the first free block large enough to service the request.
...
### Explanation of Key Mechanics

1. **Alignment Math (`((size + 7) & ~7) + 16`)**: 
   If a user requests `13` bytes, `13 + 7 = 20`. In binary, `20` is `00010100`. Masking with `~7` (`...11111000`) clears the bottom 3 bits, rounding down to `16`. We then add `16` bytes for the Header and Footer, resulting in a `32`-byte block. This guarantees the payload is perfectly 8-byte aligned.
2. **Pointer Arithmetic (`HDRP`, `FTRP`, `NEXT_BLKP`, `PREV_BLKP`)**: 
   All pointer math is done by casting the payload pointer (`bp`) to a raw byte pointer (`uint8_t*`). 
   * To find the next block, we read the size from the current block's header and jump forward by exactly that many bytes.
   * To find the previous block, we step back 16 bytes to read the *previous block's footer*, extract its size, and step back by that size to find its payload.
3. **Immediate Coalescing**:
   When `my_free` is called, it checks the allocation bit of the previous block (by looking at the word immediately preceding the current header) and the next block (by looking at the word immediately following the current footer). Depending on which neighbors are free, it merges them by writing a new, larger size into the outermost header and footer.